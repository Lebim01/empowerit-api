...
addTransactionUnconfirmed = async (
  ...
  const transactionDoc = await this.getTransactionOfUser(user_id, transactionId);
...
addTransactionConfirmed = async (
  ...
  // Comprobar si ya existe registro de la transaccion
  const { transactionId } = transactionBody.data.item;
  const transactions = await this.getTransactionOfUser(user_id, transactionId);
  ...
  calculatePendingAmount = async (
    ...
    try {
      ...
      const paidAmount:number = await this.calculatePaidAmount(id_user, addressWallet);
  ...
  calculatePaidAmount = async (
    ...
    try{
      ...
      const transactions = await this.getTransactionsOfUser(id_user, addressWallet);
